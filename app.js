const container = document.querySelector(".div-container") /* takes HTML element with the class "div-container" and storing it in the container variable. */ 
const nextBtn = document.querySelector(".next")  /* selects the HTML elements with class "next" and stores it in the nextBtn variables */
const prevBtn = document.querySelector(".previous") /* selects the HTML elements with class "previous" and stores it in the prevBtn variables */
...
const nextSlide = (inc) => { /* The nextSlide function is defined. It takes a parameter inc which represents the increment value for the slide index. */
  const slides = document.getElementsByClassName("slide")
  for (let i = 0; i < slides.length; i++) {
    slides[i].style.display = "none" /*  all the HTML elements with the class "slide" and stores them in the slides variable. 
    It then loops over each slide element and sets their display CSS property to "none", effectively hiding all the slides. */
  }
  currentIndex=(currentIndex+slides.length+inc)%slides.length; /* The currentIndex is updated by adding the inc value, which represents the increment or decrement for the slide index. The modulus operator % is used to ensure that the index remains within the valid range of slides. */
  slides[currentIndex].style.display = "block" /* the slide at the updated currentIndex is displayed by setting its display CSS property to "block". */ 
}

nextSlide(0) /* The nextSlide(0) is called initially to display the first slide. */ 

nextBtn.addEventListener("click", () => {  /* Event listeners are added to the next and previous buttons (nextBtn and prevBtn). 
...
var trekkingRoute = [
  ...
  },
  {
      ...
      location: ["USA", "Maine", "New York", "Vermont", "Maryland", "West Virginia"]
  },
  {
...
function search() {
  ...
  const RESULTS = trekkingRoute.filter(function (route) {
      return (
          ...
          route.category.some(category => category.toLowerCase().includes(searchValue)) ||
